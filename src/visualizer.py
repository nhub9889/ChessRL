...
class Visualizer:
    ...
    def update_plots(self):
        ...
        if valid_indices:
            ...
            if any(loss is not None for loss in self.metrics['supervised_loss']):
                self.axs[0, 0].plot(iterations, [self.metrics['supervised_loss'][i] for i in valid_indices], 'r-',
                                    label='Supervised Loss')
            if any(loss is not None for loss in self.metrics['value_loss']):
                self.axs[0, 0].plot(iterations, [self.metrics['value_loss'][i] for i in valid_indices], 'g-',
                                    label='Value loss')
            if any(loss is not None for loss in self.metrics['policy_loss']):
                self.axs[0, 0].plot(iterations, [self.metrics['policy_loss'][i] for i in valid_indices], 'm-',
                                    label='Policy Loss')
            self.axs[0, 0].set_title('Training loss')
            self.axs[0, 0].set_xlabel('Iterations')
            ...
            if eval_indices:
                ...
                draws = [self.metrics['eval_draws'][i] for i in eval_indices]

                self.axs[0, 1].plot(eval_iterations, wins, 'g-', label='Wins')
                self.axs[0, 1].plot(eval_iterations, draws, 'b-', label='Draws')
                self.axs[0, 1].plot(eval_iterations, losses, 'r-', label='Losses')
                self.axs[0, 1].set_title('Evaluation results')
            ...
            # Game length
            game_length_indices = [i for i, length in enumerate(self.metrics['game_length']) if length is not None]
            if game_length_indices:
                game_iterations = [self.metrics['iteration'][i] for i in game_length_indices]
                game_lengths = [self.metrics['game_length'][i] for i in game_length_indices]
    ...
    def generate_report(self):
        ...
        with open(path, 'w') as file:
            ...
            # Loss statistics
            valid_losses = [loss for loss in self.metrics['reinforcement_loss'] if loss is not None]
            if valid_losses:
    ...
    def update_metrics(self, iteration, supervised_loss=None, reinforcement_loss=None,
        ...
        if eval_results:
            ...
            self.metrics['eval_draws'].append(eval_results['draws'])
        else:
        ...
        with open(self.csv, 'a', newline='') as file:
            ...
            writer.writerow([iteration, supervised_loss, reinforcement_loss, value_loss, policy_loss,
                             eval_results['wins'] if eval_results else None,
